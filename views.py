...
def Videos(request):
    ...
    # Getting The Comming Request
    data = JSONParser().parse(request)
    if ("Vid_id" in data and data['Vid_id'] != ""):
        Videos  =int(data['vid_id'])
        maincat = Videos.objects.get(id=Videos)
        queryset = Videos.objects.filter(Category=maincat)
        serializer = Videosserilazers(queryset, many=True)
        return Response(serializer.data)
    else:
        apiresponse['Bad Request'] = 400
        return Response(apiresponse)
